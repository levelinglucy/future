# continuity_workbench_v3.py
# Pythonista local-first Continuity Workbench v3
# Added in v3:
# - backlink navigation
# - resume thread mode
# - map view (branch graph + link graph text map)
# - smarter return flow
...
class EntryForm(ui.View):
    ...
    def layout(self):
        ...
        self.body_tv.height = self.height - 420
...
class MainView(ui.View):
    def __init__(self):
        super().__init__()
        self.db = DB(DB_PATH)
        self.name = 'Continuity Workbench v3'
        self.background_color = 'white'
        ...
        labels = ['Capture', 'Entries', 'Branches', 'Return', 'Map', 'Resume', 'Snapshots']
        widths = [70, 68, 74, 66, 54, 66, 82]
        x = 8
        ...
        for v in [self.capture_view, self.entries_view, self.branches_view, self.return_view,
                  self.map_view, self.resume_view, self.snapshots_view]:
            set_frame(v, self)
            self.add_subview(v)
